...
static const u16 thumbLoadStoreRegOffsetMask = 0b1111'0000'00;
...
std::string ARM7TDMI::disassemble(u32 address, u32 opcode, bool thumb) {
	...
	if (thumb) {
		u16 lutIndex = opcode >> 8;
		if ((lutIndex & thumbAluImmediateMask) == thumbAluImmediateBits) {
...
void ARM7TDMI::halfwordDataTransfer(u32 opcode) {
	...
	if (loadStore) {
		...
		case 3: // Signed Halfword
			...
			if (address & 1)
				result = ((i32)((result >> 8) | (result << 24)) << 8) >> 8;
			break;
		default:
...
void ARM7TDMI::softwareInterrupt(u32 opcode) {
	...
	if ((opcode & 0x00FFFFFF) == 0x60000) { // Temporary HLE division to avoid thumb
		i32 numerator = reg.R[0];
		i32 denominator = reg.R[1];
		printf("Error code:  %d\n", numerator);
		reg.R[0] = (u32)(numerator / denominator);
		reg.R[1] = (u32)(numerator % denominator);
		reg.R[2] = std::abs(numerator / denominator);
	} else {
		reg.R[15] = 0x8;
		pipelineStage = 1;
		incrementR15 = false;
...
void ARM7TDMI::thumbAluImmediate(u16 opcode) {
	u32 result = opcode & 0xFF;

	switch (op) {
	case 0: // MOV
		reg.flagN = result >> 15;
		reg.flagZ = result ? false : true;
		reg.R[destinationReg] = result;
		break;
	default:
		unknownOpcodeThumb(opcode, "ALU immediate op");
		break;
	}
}
...
void ARM7TDMI::thumbHighRegOperation(u16 opcode) {
	...
	case 3: // BX
		...
		reg.R[15] = reg.R[operand2];
		pipelineStage = 1;
		incrementR15 = false;
		break;
	default:
		unknownOpcodeThumb(opcode, "High register operation");
		break;
	}
}
...
void ARM7TDMI::thumbLoadAddress(u16 opcode) {
	if (spPc) {
		...
		reg.R[destinationReg] = reg.R[15] + ((opcode & 0xFF) << 2);
	}
}

template <std::size_t lutFillIndex>
constexpr thumbLutEntry decodeThumb() {
	if ((lutFillIndex & thumbAluImmediateMask) == thumbAluImmediateBits) {
